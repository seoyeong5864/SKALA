...
class GoldenSetTest {
    ...
    private static final Logger log = LoggerFactory.getLogger(GoldenSetTest.class);
    ...
    void 골든_세트_평가() throws Exception {
        List<GoldenSet> golden = mapper.readValue(
                new ClassPathResource("golden.json").getInputStream(),
                new TypeReference<List<GoldenSet>>() {});

        int pass = 0;
        for (GoldenSet g : golden) {
            AnswerDto a = service.ask(g.q(), "golden-user", UUID.randomUUID().toString());

            boolean hit = g.must().stream().allMatch(k -> a.answer().contains(k));
            boolean cite = g.src() == null
                    || a.sources().stream().anyMatch(s -> s.document() != null && s.document().contains(g.src()));
